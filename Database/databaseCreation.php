                ...
                if ($db->exec('CREATE TABLE IF NOT EXISTS mission_type (id CHAR(36) NOT NULL PRIMARY KEY, name VARCHAR(50) NOT NULL)') !== false) {
                    ...
                    if ($db->exec('CREATE TABLE IF NOT EXISTS mission_status (id CHAR(36) NOT NULL PRIMARY KEY, name VARCHAR(50) NOT NULL)') !== false) {
                        ...
                        if ($db->exec('CREATE TABLE IF NOT EXISTS nationality (id CHAR (36) NOT NULL PRIMARY KEY, name VARCHAR(50) NOT NULL)') !== false) {
                            ...
                            if ($db->exec('CREATE TABLE IF NOT EXISTS mission (id CHAR(36) NOT NULL PRIMARY KEY, title VARCHAR(100) NOT NULL, description LONGTEXT NOT NULL, code_name VARCHAR(100) NOT NULL, country VARCHAR(50) NOT NULL, mission_start DATETIME NOT NULL, mission_end DATETIME NOT NULL, specialtity_id CHAR(36) NOT NULL, mission_type_id CHAR(36) NOT NULL, mission_status_id CHAR(36) NOT NULL, FOREIGN KEY (specialtity_id) REFERENCES speciality(id), FOREIGN KEY (mission_type_id) REFERENCES mission_type(id), FOREIGN KEY (mission_status_id) REFERENCES mission_status(id))') !== false) {
                                // Je construis maintenant ma table stash:
                                if ($db->exec('CREATE TABLE IF NOT EXISTS stash (code INTEGER(11) NOT NULL PRIMARY KEY, address VARCHAR(255) NOT NULL, country VARCHAR(100) NOT NULL, type VARCHAR(255) NOT NULL, mission_id CHAR(36), FOREIGN KEY (mission_id) REFERENCES mission(id))') !== false) {
                                    // Au tour de la table agent:
                                    if ($db->exec('CREATE TABLE IF NOT EXISTS agent (id CHAR(36) NOT NULL PRIMARY KEY, firstname VARCHAR(100) NOT NULL, lastname VARCHAR(100) NOT NULL, date_of_birth DATE NOT NULL, identity_code VARCHAR(50) NOT NULL, nationality_id CHAR(36) NOT NULL, mission_id CHAR(36) NOT NULL, FOREIGN KEY (nationality_id) REFERENCES nationality(id), FOREIGN KEY (mission_id) REFERENCES mission(id) )') !== false) {
                                        // La table speciality étant créé, la table agent étant crée, je peux faire la table associative agent_speciality:
                                        if ($db->exec('CREATE TABLE IF NOT EXISTS agent_speciality (agent_id CHAR(36) NOT NULL, speciality_id CHAR(36) NOT NULL, PRIMARY KEY(agent_id, speciality_id), FOREIGN KEY (agent_id) REFERENCES agent(id), FOREIGN KEY (speciality_id) REFERENCES speciality(id))') !== false) {
                                            // Table contact:
