        ...
        if (isset($_POST['nationalityCountryFormSubmit'])) {
            ...
            if (!empty($_POST['nationalityWritten']) && !empty($_POST['countryWritten'])) {
                ...
                if (strlen($nationalityWrittenFormated) <= 50 && strlen($countryWrittenFormated) <= 50) {
                    ...
                    // Ceci fait, je vais maintenant pouvoir enregistrer cette nationalité et son pays (avec son id) dans la base de données. Pour ce faire, il faut tout d'abord que je me connecte à la base de données. Etant donné que je souhaite que mon application tourne en production ou en local, j'utilise cette condition:
