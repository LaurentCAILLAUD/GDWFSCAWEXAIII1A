...
if (!isset($_SESSION['userEmail']) || $_SESSION['userRole'] != 'ROLE_ADMIN') {
    ...
    try {
        ...
        if (isset($_GET['confirm'])) {
            ...
            if ($_GET['confirm'] == 'yes') {
                // Afin de supprimer ma cible je vais avoir besoin de me connecter à la base de données:
                ...
                // Et j'utilise la fonction deleteThisTargetWithThisId() afin de supprimer ma cible:
                $targetRepository->deleteThisTargetWithThisId($_GET['id']);
                // Si une erreur se déroule dans la suppression de la cible une erreur est levée. Si au contraire cette suppression se passe bien je dirige l'administrateur vers la page qui liste les cibles et où il verra que la suppression s'est bien faite:
