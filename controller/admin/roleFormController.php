        ...
        if (isset($_POST['roleFormSubmit'])) {
            ...
            if (!empty($_POST['roleWritten'])) {
                ...
                if (strlen($roleWritten) <= 50) {
                    ...
                    // Ceci fait, je vais maintenant pouvoir enregistrer ce rôle (avec son id) dans la base de données. Pour ce faire, il faut tout d'abord que je me connecte à la base de données. Etant donné que je souhaite que mon application tourne en production ou en local, j'utilise cette condition:
