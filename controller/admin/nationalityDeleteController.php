...
if (!isset($_SESSION['userEmail']) || $_SESSION['userRole'] != 'ROLE_ADMIN') {
    ...
    try {
        ...
        if (isset($_GET['confirm'])) {
            ...
            if ($_GET['confirm'] == 'yes') {
                // Afin de supprimer ma nationalité je vais avoir besoin de me connecter à la base de données:
                $dsn = 'mysql:host=localhost;dbname=GDWFSCAWEXAIII1A';
                //Je me connecte à la base de données:
                $db = new PDO($dsn, 'root', 'root');
                // J'instancie un nouvel objet de ma classe NationalityRepository:
                $nationalityRepository = new NationalityRepository($db);
                // Et j'utilise la fonction deleteThisNationalityWithThisId() afin de supprimer ma nationalité:
                $nationalityRepository->deleteThisNationalityWithThisId($_GET['id']);
        ...
        $nationalityDeleteViewMessage = $exception->getMessage();
    }
}
