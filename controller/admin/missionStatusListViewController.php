...
if (!isset($_SESSION['userEmail']) || $_SESSION['userRole'] != 'ROLE_ADMIN') {
    ...
    try {
        // Le but de ce contrôleur est de récupérer les statuts de mission enregistrés dans la base de données. Pour cela, il va falloir que je me connecte à la base de données. Etant donné que je souhaite que mon application tourne en production ou en local, j'utilise cette condition:
        ...
        // Afin de récupérer les données, je vais utiliser la classe MissionStatusRepository et plus particulièrement sa fonction getAllStatusMission().
        ...
        $allMissionsStatus = $missionStatusRepository->getAllStatusMission();
    } catch (Exception $exception) {
        $missionStatusListViewMessage = $exception->getMessage();
    }
}
