...
if (!isset($_SESSION['userEmail']) || $_SESSION['userRole'] != 'ROLE_ADMIN') {
    ...
    try {
        // Le but de ce contrôleur est de récupérer les nationalités enregistrés dans la base de données. Pour cela, il va falloir que je me connecte à la base de données et il me faut donc dans un premier temps créé mon Data Source Name:
        $dsn = 'mysql:host=localhost;dbname=GDWFSCAWEXAIII1A';
        // Celui-ci créé, je peux maintenant me connecter à la base de données:
        $db = new PDO($dsn, 'root', 'root');
        // Afin de récupérer les données, je vais utiliser la classe NationalityRepository et plus particulièrement sa fonction getAllNationalities().
        ...
        $allNationalities = $nationalityRepository->getAllNationalities();
    } catch (Exception $exception) {
        $nationalityListViewMessage = $exception->getMessage();
    }
}
