        ...
        if (isset($_POST['missionStatusFormSubmit'])) {
            ...
            if (!empty($_POST['missionStatusWritten'])) {
                ...
                if (strlen($missionStatusWrittenFormated) <= 30) {
                    ...
                    // Ceci fait, je vais maintenant pouvoir enregistrer ce statut de mission (avec son id) dans la base de données. Pour ce faire, il faut tout d'abord que je me connecte à la base de données. Etant donné que je souhaite que mon application tourne en production ou en local, j'utilise cette condition:
