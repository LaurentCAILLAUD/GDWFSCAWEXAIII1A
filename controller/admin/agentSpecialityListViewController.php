...
if (!isset($_SESSION['userEmail']) || $_SESSION['userRole'] != 'ROLE_ADMIN') {
    ...
    try {
        // Le but de ce contrôleur est de récupérer tous les agents avec leur ou leurs spécialité(ées). Pour cela, il va falloir que je me connecte à la base de données. Etant donné que je souhaite que mon application tourne en production ou en local, j'utilise cette condition:
        ...
        // Afin de récupérer les données, je vais utiliser la classe AgentSpecialityRepository et plus particulièrement sa fonction getAllAgentIdentitiesWithSpecialities().
        ...
        $allAgentIdentitiesWithSpecialities = $agentSpecialityRepository->getAllAgentIdentitiesWithSpecialities();
    } catch (Exception $exception) {
        $agentSpecialityListViewMessage = $exception->getMessage();
    }
}
