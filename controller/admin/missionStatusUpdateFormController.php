...
if (!isset($_SESSION['userEmail']) || $_SESSION['userRole'] != 'ROLE_ADMIN') {
    ...
    try {
        // Afin d'afficher le statut de mission que l'administrateur souhaite modifier dans le champ du formulaire, il faut que j'aille le récupérer grâce à son id (id présent dans l'url de la requête). Pour cela je vais devoir me connecter à la base de données. Etant donné que je souhaite que mon application tourne en production ou en local, j'utilise cette condition:
