...
if (!isset($_SESSION['userEmail']) || $_SESSION['userRole'] != 'ROLE_ADMIN') {
    ...
    try {
        ...
        if (isset($_POST['missionUpdateFormSubmit'])) {
            ...
            if (!empty($_POST['titleWritten']) && !empty($_POST['codeNameWritten']) && !empty($_POST['descriptionWritten']) && !empty($_POST['countryWritten']) && !empty($_POST['missionStartSelected']) && !empty($_POST['missionEndSelected']) && !empty($_POST['specialityIdSelected']) && !empty($_POST['missionTypeIdSelected']) && !empty($_POST['missionStatusIdSelected'])) {
                ...
                if (strlen($titleWrittenFormated) <= 100 && strlen($codeNameWrittenFormated) <= 100) {
                    // Il faut maintenant que je fasse la même chose pour le champ du pays:
                    if (strlen($countryWrittenFormated) <= 50) {
                        // Les saisies de notre utilisateur sont maintenant sécurisées et dans le bon format. Je vais pouvoir maintenant enregistrer celles-ci dans la base de données. Pour ce faire je vais dans un premier temps créer un nouvel objet de ma classe Mission:
                        $mission = new Mission($_GET['id'], $titleWrittenFormated, $descriptionWrittenFormated, $codeNameWrittenFormated, $countryWrittenFormated, $missionStart, $missionEnd, $_POST['specialityIdSelected'], $_POST['missionTypeIdSelected'], $_POST['missionStatusIdSelected']);
                        ...
                        throw new Exception('Le pays de votre mission ne doit pas dépasser 50 caractères.');
                    }
                } else {
