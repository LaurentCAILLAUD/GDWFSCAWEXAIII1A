...
if (!isset($_SESSION['userEmail']) || $_SESSION['userRole'] != 'ROLE_ADMIN') {
    ...
    try {
        // Le but de ce contrôleur est de récupérer les types de missions enregistrés dans la base de données. Pour cela, il va falloir que je me connecte à la base de données. Etant donné que je souhaite que mon application tourne en production ou en local, j'utilise cette condition:
        ...
        // Afin de récupérer les données, je vais utiliser la classe MissionTypeRepository et plus particulièrement sa fonction getAllMissionType(.)
        ...
        $allMissionsType = $missionTypeRepository->getAllMissionType();
    } catch (Exception $exception) {
        $missionTypeListViewMessage = $exception->getMessage();
    }
}
